...
public class Subway_Navi {
    public static void main(String[] args) {
        ...
        System.out.print("출발 역을 입력하세요 ex) 서울역(1), 압구정(3) : ");
        String start = scanner.nextLine();
        System.out.print("도착 역을 입력하세요 ex) 서울역(1), 압구정(3) : ");
        String end = scanner.nextLine();

        Subway_Map map = new Subway_Map();
        Subway_Map.result result = map.Map(subwayMap, start, end);

        if(result.time == -1)
            System.out.println("경로를 찾을 수 없습니다.");
        else {
            while (!result.station.isEmpty()) {
                if (result.station.peek().equals(end)) System.out.println(result.station.pop());
                else System.out.print(result.station.pop() + " -> ");
            }
            System.out.println("총 소요시간 : " + result.time + "분");
        }
    }
}
